...
### Scaling Factors
To map generic or commercial performance charts directly onto the calculated cycle's design point, linear scaling factors are established at the reference line. Note that text labels inside subscripts (such as $\text{unscaled,dp}$) are properly wrapped in standard text blocks to avoid rendering glitches:

* **Compressor Scaling Factors:**
  $$f_{m,C} = \frac{\dot{m}_{C,dp}}{\dot{m}_{C,\text{unscaled,dp}}}$$
  $$f_{PR,C} = \frac{\pi_{C,dp} - 1}{\pi_{C,\text{unscaled,dp}} - 1}$$
  $$f_{\eta,C} = \frac{\eta_{C,dp}}{\eta_{C,\text{unscaled,dp}}}$$

* **Turbine Scaling Factors:**
  $$f_{m,T} = \frac{\dot{m}_{T,dp}}{\dot{m}_{T,\text{unscaled,dp}}}$$
  $$f_{PR,T} = \frac{(1/\pi_{T,dp}) - 1}{\pi_{T,\text{unscaled,dp}} - 1}$$
  $$f_{\eta,T} = \frac{\eta_{T,dp}}{\eta_{T,\text{unscaled,dp}}}$$
...
$$X = \begin{bmatrix} \dot{m}_{C,corr} \\ N_{C,\text{perc}} \\ \pi_T \end{bmatrix}$$
...
1. **Turbine Mass Flow Continuity Error ($f_1$):**
   ...
   $$\dot{m}_{T,\text{cycle}} = \dot{m}_{a,OD} \cdot (1 + f)$$
   $$\dot{m}_{T,\text{map}} = \text{Turbine.Interpolate}\left(N_{T,corr}, \pi_T\right) \cdot \frac{p_{3,0}/p_{ref}}{\sqrt{T_{3,0}/T_{ref}}}$$
   $$f_1 = \frac{\dot{m}_{T,\text{map}} - \dot{m}_{T,\text{cycle}}}{\dot{m}_{T,dp}} = 0$$
...
$$J = \begin{bmatrix} 
\frac{\partial f_1}{\partial \dot{m}_{C,corr}} & \frac{\partial f_1}{\partial N_{C,\text{perc}}} & \frac{\partial f_1}{\partial \pi_T} \\ 
\frac{\partial f_2}{\partial \dot{m}_{C,corr}} & \frac{\partial f_2}{\partial N_{C,\text{perc}}} & \frac{\partial f_2}{\partial \pi_T} \\ 
\frac{\partial f_3}{\partial \dot{m}_{C,corr}} & \frac{\partial f_3}{\partial N_{C,\text{perc}}} & \frac{\partial f_3}{\partial \pi_T}
\end{bmatrix}$$
